...
export default function EditCourse() {
  ...
  const fetchCourse = useCallback(async () => {
    try {
      ...
      setFormData({
        ...
        benefits:
          course.benefits.length > 0 ? course.benefits : [{ title: "" }],
        prerequisites:
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await api.put(`/edit-course/${params.id}`, formDataToSend, {
        headers: {
          "Content-Type": "multipart/form-data",
        },
      });
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center space-x-4">
        ...
        <div>
          ...
          <p className="text-gray-400">Update course information and content</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div className="bg-gray-800/50 backdrop-blur-sm border border-gray-700/50 rounded-xl p-6">
          ...
          <div className="space-y-4">
            {thumbnailPreview && (
              <div className="relative w-48 h-32 bg-gray-700 rounded-lg overflow-hidden">
                <Image
            ...
            <div className="border-2 border-dashed border-gray-600/50 rounded-lg p-6">
              <div className="text-center">
                <Upload className="h-8 w-8 text-gray-400 mx-auto mb-2" />
                <p className="text-gray-400 text-sm mb-2">
                  Upload a new thumbnail image (JPEG, PNG, GIF, WebP)
                </p>
                <input
                  type="file"
                  accept="image/*"
        ...
        <div className="flex items-center justify-end space-x-4">
          <button
            type="button"
            onClick={() => router.back()}
            className="px-6 py-3 bg-gray-700/50 hover:bg-gray-600/50 text-white rounded-xl transition-colors"
          >
            Cancel
          </button>
          <button
            type="submit"
            disabled={saving}
            className="flex items-center space-x-2 bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 text-white py-3 px-6 rounded-xl font-semibold transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {saving ? (
